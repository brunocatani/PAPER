...
Require-Text 'src/PaperMain.cpp' 'refreshEnrichmentDemand[\s\S]*ReloadAnimationEvidence[\s\S]*ReloadObservations[\s\S]*ReloadEvidenceGeometry[\s\S]*enrichmentDemand\.reloadObservation[\s\S]*enrichmentDemand\.animationEvidence' 'Reload observation and animation enrichment must remain dormant unless a consumer negotiates the corresponding raw-data capability.'
Require-Text 'src/api/PAPERApi.h' 'PAPER_API_VERSION\s*=\s*1[\s\S]*ReloadAnimationEvidence[\s\S]*PAPER_PROVIDER_API_V1_RELOAD_OBSERVATION_TABLE_BYTES[\s\S]*PAPER_PROVIDER_API_V1_RELOAD_ANIMATION_TABLE_BYTES' 'Animation evidence must be an additive V1 table extension that freezes the prior reload-observation extent.'
Require-Text 'src/api/PAPERProvider.cpp' 'ReloadAnimationEvidence[\s\S]*getReloadAnimationCatalogStateV1[\s\S]*copyReloadAnimationSamplesV1[\s\S]*copyReloadAnimationSkeletonV1' 'Every raw animation query must remain owner-bound and capability-gated.'

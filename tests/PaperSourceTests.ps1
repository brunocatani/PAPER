...
Require-Text 'src/animation/NativeAnimationAuthorityPolicy.h' 'canApplyNativeAnimationForFiringHand[\s\S]*!eligibility\.gripStateValid[\s\S]*!eligibility\.firingHandIsLeft' 'PAPER must fail closed when ROCK confirms a physical-left firing hand while preserving no-grip lifecycle behavior.'
Require-Text 'src/PaperMain.cpp' 'observeFiringHandCompatibility[\s\S]*canApplyNativeAnimationForFiringHand[\s\S]*runtimeOperational[\s\S]*handCompatible[\s\S]*AfterRock[\s\S]*!handCompatible\s*&&\s*s_runtimeOperational[\s\S]*configureRuntime\(false\)[\s\S]*publishRockAuthorityFlags\(0\)' 'Both PAPER phases must suppress and clear right-authored pose authority when a handoff makes physical left the firing hand.'

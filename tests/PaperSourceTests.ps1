...
Require-Text 'data/config/PAPER.ini' '\[WeaponMotionCache\][\s\S]*bEnabled\s*=\s*true[\s\S]*iSessionCacheMiB\s*=\s*64[\s\S]*iDiskCacheMiB\s*=\s*256[\s\S]*iMaximumFileMiB\s*=\s*16[\s\S]*iMaximumEntries\s*=\s*256' 'The compiled motion cache must retain conservative enabled-by-default storage bounds.'
...
Require-Text 'src/PaperMain.cpp' 'refreshEnrichmentDemand[\s\S]*WeaponMotionCatalog[\s\S]*WeaponMotionDiagnostics[\s\S]*WeaponManipulationTelemetry[\s\S]*requiresExactAnimationEvidence[\s\S]*reloadObservation\s*=\s*weaponMotion[\s\S]*weapon_motion::reset[\s\S]*Complete[\s\S]*animation_evidence::completeFrame[\s\S]*weapon_motion::completeFrame[\s\S]*drainEvents[\s\S]*dispatchEvent' 'Motion intelligence must be dormant without consumers, allow a validated compiled-cache hit to suppress exact preharvest, and publish only after its raw inputs complete.'
...
Require-Text 'src/weapon_motion/WeaponMotion.cpp' 'CacheLookupPending[\s\S]*hydrateCachedRecord[\s\S]*HydratedFromCache[\s\S]*queueCompletedCacheRecord' 'Compiled authored motion must remain asynchronously validated, live-rebased, and explicitly identified when served from cache.'
Require-Text 'src/weapon_motion/WeaponMotion.cpp' 'void activate\(\)[\s\S]*s_cacheStore->start\(\)[\s\S]*s_runtime' 'The compiled cache worker must remain dormant until weapon-motion consumer demand activates the subsystem.'
Require-Text 'src/weapon_motion/WeaponMotionCache.cpp' 'writeFileAtomically[\s\S]*environmentFingerprint[\s\S]*kCompilerSchemaVersion' 'The PAPER-owned compiled cache must retain schema/environment invalidation and atomic persistence.'
Require-Text 'src/weapon_motion/WeaponMotionCache.cpp' 'maximumSessionBytes[\s\S]*maximumDiskBytes' 'The PAPER-owned compiled cache must enforce bounded session and disk storage.'
Require-Text 'src/weapon_motion/WeaponMotionCache.h' 'maximumSessionBytes[\s\S]*maximumDiskBytes[\s\S]*maximumFileBytes[\s\S]*maximumEntries' 'Compiled cache storage limits must remain explicit configuration inputs.'
Reject-Text 'src/weapon_motion/WeaponMotionCache.cpp' 'PaperReloadAnimationSampleV1|hkbBehaviorGraph|hkaAnimation|hkRefPtr' 'The persistent cache must never retain raw animation samples or native graph/clip ownership.'

...
Require-Text 'src/PaperMain.cpp' 'copyWeaponPartPoses[\s\S]*isManualCycleSemanticActionPart[\s\S]*isManualCycleActionPartMotion[\s\S]*semanticManualCycleActionPresent[\s\S]*semanticManualCycleActionMoved' 'Manual-cycle admission and activation must consume generation-bound ROCK pump, lever, bolt, cylinder, and break-action evidence.'
Require-Text 'src/animation/NativeAnimationAuthority.cpp' 'onWeaponFire[\s\S]*s_localManualCycleCandidateRequestSequence[\s\S]*s_localManualCycleCandidatePending[\s\S]*refreshLocalManualCycleCandidate[\s\S]*advanceLocalManualCycleCandidate[\s\S]*s_localManualCycleTestLeaseActive' 'A fire event must create a bounded candidate before it can activate hand authority.'
Require-Text 'src/animation/NativeAnimationAuthority.cpp' 'ActivateReloadEnd[\s\S]*native reload-end marker[\s\S]*ActivateActionPartMotion[\s\S]*ROCK semantic action-part motion' 'Manual-cycle candidates must identify both native cycle-marker and ROCK semantic action-part activation evidence.'
...
Require-Text 'src/PaperMain.cpp' 'SupportFullAuthority[\s\S]*SupportVisualOnly[\s\S]*queryWeaponPartGripState[\s\S]*leftPartGripStateValid[\s\S]*WeaponRootLocal' 'Manual-cycle support-hand animation must observe the active current-weapon support grip.'
Require-Text 'src/PaperMain.cpp' 'leftSupportCandidate[\s\S]*snapshot\.leftSupportGripValid\s*=\s*true[\s\S]*snapshot\.authoredLeftActive\s*=[\s\S]*leftPartGrip\.authoredSupportGrip\s*!=\s*0' 'Manual-cycle support-hand animation must consume ROCK''s exact authored-grip provenance.'
...
Require-Text 'src/animation/NativeAnimationAuthority.cpp' 'leftSupportGripValid[\s\S]*authoredLeftActive[\s\S]*resolved\.leftHandInWeapon[\s\S]*resolved\.authoredLeftActive\s*=\s*true' 'Manual-cycle authored support-grip admission must trust ROCK''s generation-bound provenance instead of transform-nearness guessing.'
Reject-Text 'src/animation/NativeAnimationAuthority.cpp' 'isAuthoredSupportGripMatch|kAuthoredSupportGripTranslationTolerance|kAuthoredSupportGripRotationTolerance' 'Paper must not reconstruct authored support-grip ownership from transform tolerances.'
...
Require-Text 'src/animation/NativeAnimationAuthority.cpp' 'cancelLocalReloadTestLease\(\)[\s\S]{0,320}s_playerReloadEventActive\.store\([\s\S]{0,80}false[\s\S]*refreshLocalReloadTestLease[\s\S]*cancelLocalReloadTestLease\(\)' 'Cancelled or expired reload leases must not leave a stale reload-active latch that blocks later manual cycles.'
Reject-Text 'src/PaperMain.cpp' 'queryEquippedWeaponState|TransitionActive|IdentityPending|DrawPending|setEquippedWeaponAnimationContext' 'Native animation authority must not depend on the removed equipped/drawn transition gate.'
Reject-Text 'src/animation/NativeAnimationAuthority.cpp' 'nativeAnimationWeaponContextAllowsAuthority|nativeWeaponIsDrawn|NativeAnimationWeaponContextGate' 'Native reload and manual-cycle authority must retain the ungated runtime flow.'
...
Require-Text 'src/api/RockApiClient.cpp' 'PoseReadback[\s\S]*WeaponPartObservability[\s\S]*supportsWeaponPartGripStateV1[\s\S]*supportsWeaponPartObservabilityV1[\s\S]*getWeaponPartGripStateV1[\s\S]*copyWeaponPartPoseSnapshotV1' 'Paper must negotiate and consume ROCK''s V1 grip and semantic part-pose readback surfaces.'

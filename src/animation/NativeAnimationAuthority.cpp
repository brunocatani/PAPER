...
namespace paper::native_animation_authority
{
    ...
    {
        if (s_cycleTrace) {
            const bool tracing = s_runtimeEnabled.load(std::memory_order_acquire) &&
                !s_cycleTrace->failed.load(std::memory_order_relaxed);
            const auto& identity = s_latestManualCycleRockGripBaselines;
            clip_telemetry::refreshHandActionTrace(
                tracing ? identity.weaponFormId : 0, identity.weaponGenerationKey,
                identity.weaponNode, s_cycleTrace->log);
        }
    ...
    {
        clip_telemetry::refreshHandActionTrace(0, 0, 0, {});

...
namespace paper::clip_telemetry
{
    namespace
    {
        ...
        constexpr std::uintptr_t kClipGeneratorActivateFunctionOffset =
            0x192CA40;
        constexpr std::uintptr_t kClipGeneratorUpdateFunctionOffset =
            0x192D0D0;
        constexpr std::uintptr_t kClipGeneratorDeactivateFunctionOffset =
            0x192D510;
        ...
        /*
         * Annotation/trigger offsets, verified 2026-07-05 via the
        ...
        /*
         * Per-weapon-generation line budget for raw marker diagnostics. The
         ...
         */
        constexpr std::uint32_t kMarkerLogBudgetPerGeneration = 160;
        ...
        std::array<CapturePacket, kRichClipQueueCapacity> s_richClipQueue{};
        ...
        struct MarkerCaptureScratch
        {
            ...
            std::array<CapturedAnnotation, animation_evidence::kMaxAnnotationsPerClip> annotations{};
            std::array<CapturedTrigger, animation_evidence::kMaxTriggersPerClip> triggers{};
        };
        ...
        std::array<std::int16_t, animation_evidence::kMaxLiveTracksPerClip> s_scratchTrackIndices{};
        std::array<animation_evidence::LiveTrackSamples, animation_evidence::kMaxLiveTracksPerClip> s_scratchTracks{};

        std::atomic<std::uint64_t> s_bindingsSeen{ 0 };
        std::atomic<std::uint64_t> s_bindingsCaptured{ 0 };
        std::atomic<std::uint64_t> s_bindingsWithoutTargets{ 0 };
        ...
        std::atomic<std::uint64_t> s_rejectedAnimationPointer{ 0 };
        std::atomic<std::uint64_t> s_rejectedClipParameters{ 0 };
        std::atomic<std::uint64_t> s_rejectedTrackMap{ 0 };
        std::atomic<std::uint64_t> s_rejectedBoneCount{ 0 };
        std::atomic<std::uint64_t> s_rejectedSampler{ 0 };
        std::atomic<std::uint64_t> s_rejectedSplineData{ 0 };
        ...
        std::atomic<std::uint64_t> s_hookCalls{ 0 };
        std::atomic<std::uint64_t> s_hookMatches{ 0 };
        ...
        struct ProcessedBinding
        {
            ...
            // Repeat activations reuse it, so published samples never point
            // at an activity id absent from the per-weapon catalog.
        ...
        std::uint32_t s_markerLogBudget = kMarkerLogBudgetPerGeneration;
        ...
        struct RichActiveClip
        {
            ...
            std::array<char, animation_evidence::kBoneNameCapacity> name{};
            float duration{ 0.0f };
        ...
        // reported by the caller when a walk gives up so the failing point
        // is visible in the log instead of a generic timeout.
        const char* s_lastResolvePoint = "none";
        ...
        /*
         * RAW ANIMATION MARKERS: the engine's authored timing records are
         * copied without interpretation. Two kinds:
            ...
            MarkerCaptureScratch* capture)
        {
            ...
            if (s_markerLogBudget == 0 && !capture) {
                return;
            }
            ...
            const MarkerCaptureScratch* markers = nullptr)
        {
            ...
            const auto animation = *reinterpret_cast<std::uintptr_t*>(binding + kBindingAnimationOffset);
            if (!plausiblePointer(animation)) {
                s_rejectedAnimationPointer.fetch_add(1, std::memory_order_relaxed);
            ...
            if (!std::isfinite(duration) || duration < kMinClipDurationSeconds || duration > kMaxClipDurationSeconds ||
                ...
                s_rejectedClipParameters.fetch_add(1, std::memory_order_relaxed);
            ...
            if (splineNumBlocks <= 0 || splineMaxFramesPerBlock < 2 ||
                ...
                s_rejectedSplineData.fetch_add(1, std::memory_order_relaxed);
                if (claimBindingDetailLog()) {
                    PAPER_LOG_WARN(Weapon,
            ...
            if (boneCount <= 0 || boneCount > kMaxPlausibleBoneCount) {
                s_rejectedBoneCount.fetch_add(1, std::memory_order_relaxed);
            ...
            if (targetCount == 0) {
                s_bindingsWithoutTargets.fetch_add(1, std::memory_order_relaxed);
                return true;
            }
            if (outHadWeaponTracks) {
            ...
            if (!sampler) {
                s_rejectedSampler.fetch_add(1, std::memory_order_relaxed);
            ...
            for (std::uint32_t step = 0; step < animation_evidence::kLiveClipSampleCount; ++step) {
                const float time = duration * static_cast<float>(step) /
                                   static_cast<float>(animation_evidence::kLiveClipSampleCount - 1);
                ...
                for (std::uint32_t i = 0; i < targetCount; ++i) {
                    ...
                    pose.translate = animation_evidence::Vec3{
                        decoded.translate[0],
                        decoded.translate[1],
                        decoded.translate[2],
                    };
                    // Havok quaternion order is (x, y, z, w).
                    pose.rotate = animation_evidence::normalizedQuaternionOrIdentity(animation_evidence::Quat{
                        decoded.rotate[3],
                        decoded.rotate[0],
                        decoded.rotate[1],
                        decoded.rotate[2],
                    });
                    tracks[i].scales[step] = animation_evidence::Vec3{
                        decoded.scale[0],
                        decoded.scale[1],
                        decoded.scale[2],
                    };
            ...
            if (s_richCaptureEnabled.load(std::memory_order_relaxed)) {
                ...
                if (s_richCaptureEnabled.load(std::memory_order_relaxed)) {
                    ...
                    if (packet) {
                        ...
                        static_assert(std::is_trivially_copyable_v<CapturePacket>);
                        ...
                        packet->acquisition = fromActivation
                            ? animation_evidence::ClipAcquisition::LiveClipActivation
        ...
        {
            s_lastResolvePoint = "manager";
            if (!plausiblePointer(manager)) {
                return false;
            }
            s_lastResolvePoint = "graphs-array";
        ...
        {
            s_lastResolvePoint = "graph";
            if (!graph) {
                return false;
            }
            s_lastResolvePoint = "character-setup";
            ...
            std::uint64_t activityId)
        {
            ...
            if (!slot) {
                if (s_richActiveClipOverflowLogs.fetch_add(1, std::memory_order_relaxed) < 3) {
                    PAPER_LOG_WARN(Weapon,
                        "WeaponClipTelemetry: active clip registry full; clip {:#x} has no activity correlation slot",
                        clipGenerator);
                }
                return;
            }
        ...
        void captureFromClipGenerator(void* clipGeneratorRaw, void* contextRaw)
        {
            ...
            s_hookMatches.fetch_add(1, std::memory_order_relaxed);
            ...
            // Markers are copied before track capture so names/times remain
            ...
            auto* markerCapture = s_richCaptureEnabled.load(std::memory_order_relaxed)
                ...
                : nullptr;
            captureClipMarkers(clipGenerator, binding, animationName.data(), behaviorGraph, markerCapture);
    ...
    bool ensureHooksInstalled()
    {
        ...
        if (!REL::Module::IsVR() ||
            ...
            PAPER_LOG_ERROR(
                Init,
                "Passive clip telemetry requires verified Fallout4VR.exe 1.2.72 layouts");
            return false;
        }
        ...
        std::uintptr_t activateFunction = 0;
        std::uintptr_t updateFunction = 0;
        std::uintptr_t deactivateFunction = 0;
        if (!native_memory::tryReadValue(
                reinterpret_cast<const std::uintptr_t*>(activateSlot),
                activateFunction) ||
            !native_memory::tryReadValue(
                reinterpret_cast<const std::uintptr_t*>(updateSlot),
                updateFunction) ||
            !native_memory::tryReadValue(
                reinterpret_cast<const std::uintptr_t*>(deactivateSlot),
                deactivateFunction) ||
            activateFunction != REL::Module::get().base() +
                kClipGeneratorActivateFunctionOffset ||
            updateFunction != REL::Module::get().base() +
                kClipGeneratorUpdateFunctionOffset ||
            deactivateFunction != REL::Module::get().base() +
                kClipGeneratorDeactivateFunctionOffset) {
            PAPER_LOG_ERROR(
                Init,
                "Passive clip telemetry hook validation failed; lifecycle vtable entries do not match Fallout4VR.exe 1.2.72");
            return false;
        }
        s_originalClipActivate = reinterpret_cast<ClipGeneratorActivateFn>(
            activateFunction);
        REL::safe_write(activateSlot, reinterpret_cast<std::uintptr_t>(&clipGeneratorActivateShim));
        s_originalClipUpdate = reinterpret_cast<ClipGeneratorUpdateFn>(
            updateFunction);
        REL::safe_write(updateSlot, reinterpret_cast<std::uintptr_t>(&clipGeneratorUpdateShim));
        s_originalClipDeactivate = reinterpret_cast<ClipGeneratorDeactivateFn>(
            deactivateFunction);
        REL::safe_write(deactivateSlot, reinterpret_cast<std::uintptr_t>(&clipGeneratorDeactivateShim));
        std::uintptr_t installedActivate = 0;
        std::uintptr_t installedUpdate = 0;
        std::uintptr_t installedDeactivate = 0;
        if (!native_memory::tryReadValue(
                reinterpret_cast<const std::uintptr_t*>(activateSlot),
                installedActivate) ||
            !native_memory::tryReadValue(
            ...
            !native_memory::tryReadValue(
                reinterpret_cast<const std::uintptr_t*>(deactivateSlot),
                installedDeactivate) ||
            installedActivate != reinterpret_cast<std::uintptr_t>(
                &clipGeneratorActivateShim) ||
            installedUpdate != reinterpret_cast<std::uintptr_t>(
                &clipGeneratorUpdateShim) ||
            installedDeactivate != reinterpret_cast<std::uintptr_t>(
                &clipGeneratorDeactivateShim)) {
            REL::safe_write(activateSlot, activateFunction);
            REL::safe_write(updateSlot, updateFunction);
            REL::safe_write(deactivateSlot, deactivateFunction);
            s_originalClipActivate = nullptr;
            s_originalClipUpdate = nullptr;
            s_originalClipDeactivate = nullptr;
            PAPER_LOG_ERROR(
                Init,
                "Passive clip telemetry hook installation did not verify; original vtable entries restored");
            return false;
        }
        s_installed = true;
        PAPER_LOG_INFO(Weapon,
            ...
            if (s_walkGraphIndex >= graphs.capacity) {
                ...
                if (s_walkPassIndex == 0 || captured != s_walkPassStartCaptured) {
                    PAPER_LOG_INFO(Weapon,
                        ...
                        captured,
                        s_bindingsWithoutTargets.load(std::memory_order_relaxed));
                }
                ++s_walkPassIndex;
                return StepResult::Completed;
            }
    ...
    std::uint32_t drainCaptures(CapturePacket* outPackets, std::uint32_t maxPackets)
    {
        ...
        if (count < s_richClipQueueCount) {
            ...
            std::memmove(
                ...
                sizeof(CapturePacket) * (s_richClipQueueCount - count));
        }
        s_richClipQueueCount -= count;
        return count;
    }

    DropInfo drainDropInfo()
    {
        std::scoped_lock lock(s_richClipQueueMutex);
        DropInfo info{
    ...
    Stats snapshotStats()
    {
        return Stats{
            ...
            .bindingsWithoutTargets = s_bindingsWithoutTargets.load(std::memory_order_relaxed),
            ...
            .rejectedAnimationPointer = s_rejectedAnimationPointer.load(std::memory_order_relaxed),
            .rejectedClipParameters = s_rejectedClipParameters.load(std::memory_order_relaxed),
            .rejectedTrackMap = s_rejectedTrackMap.load(std::memory_order_relaxed),
            .rejectedBoneCount = s_rejectedBoneCount.load(std::memory_order_relaxed),
            .rejectedSampler = s_rejectedSampler.load(std::memory_order_relaxed),
            .rejectedSplineData = s_rejectedSplineData.load(std::memory_order_relaxed),
            .hookCalls = s_hookCalls.load(std::memory_order_relaxed),
            .hookMatches = s_hookMatches.load(std::memory_order_relaxed),
        };
    }
}

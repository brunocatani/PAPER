...
namespace paper::native_memory
{
            ...
            bool (*allowsProtection)(DWORD))
        {
            ...
            while (current < end) {
                ...
                if (VirtualQuery(
                    ...
                    !allowsProtection(memoryInfo.Protect)) {
                    return false;
                }
    ...
    bool pointerRangeLooksReadable(
        const void* pointer,
        const std::size_t byteCount)
    {
        return rangeHasProtection(
            pointer,
            byteCount,
            pageProtectionAllowsRead);
    }
        ...
        const std::size_t byteCount)
    {
        return rangeHasProtection(
            pointer,
            byteCount,
            pageProtectionAllowsWrite);
    }

    bool guardedCopyFromMemory(
        const void* source,
        void* target,
        const std::size_t byteCount)
    {
        if (!source || !target || byteCount == 0 ||
            !pointerRangeLooksReadable(source, byteCount)) {
            return false;
        }
#if defined(_MSC_VER)
        __try {
